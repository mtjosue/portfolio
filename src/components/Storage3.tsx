...
const SecondHome = () => {
  ...
  const { opacity, transform } = useSpring({
    ...
    opacity: conPosition > 130 ? 1 : 0,
    transform: `translate(${Math.min(
      conPosition * 6,
      width / 2 - elWidth,
    )}px, ${Math.min(conPosition > 130 ? conPosition * 6 : 0, 80)}px) rotate(${
      180 + Math.min(conPosition > 130 ? conPosition * 2 : 0, 180)
    }deg)`,

    // transform: `translate(50vw, 50vh) rotate(${
        ...
        <div className="flex max-w-[16rem]">
          ...
          >
            <span className="font-bold">
              ...
              a journey of growth, aiming to be of value to others. */}
            </span>
          </animated.div>
        </div>
